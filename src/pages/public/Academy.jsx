    ...
    <div>
      <section className="brand-gradient-bg text-white py-16 sm:py-20">
        <div className="container-app text-center">
          <span className="text-xs font-bold uppercase tracking-widest text-brand-200">RiseDigital Academy</span>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((c) => (
              <Link
                key={c.id}
                to={`/academy/${c.id}`}
                className="group relative bg-white border-2 border-surface-alt rounded-2xl p-6 hover:border-brand-400 hover:shadow-2xl hover:shadow-brand-500/10 transition-all duration-300 hover:-translate-y-1 flex flex-col"
              >
                ...
                <p className="text-sm text-navy-600 leading-relaxed mb-5 flex-1">{c.description}</p>

                <div className="flex items-center justify-between border-t border-surface-alt pt-4 mb-5">
                  ...
                  <span className="text-lg font-bold font-display text-navy-900">{c.price}</span>
                </div>

                <span className="inline-flex items-center justify-center gap-1.5 w-full rounded-full border-2 border-navy-900 text-navy-900 font-semibold text-sm py-2.5 group-hover:bg-navy-900 group-hover:text-white transition-colors">
